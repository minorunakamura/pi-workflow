...
function createLayout() {
  const root = resolve(
    mkdtempSync(join(tmpdir(), "pi-workflow-unit6-native-")),
  );
  const layout = {
    root,
    home: join(root, "home"),
  ...
  for (const directory of Object.values(layout)) {
    if (
      directory !== root &&
      !directory.endsWith(".log") &&
      !directory.endsWith(".err") &&
      !directory.endsWith(".json") &&
      !directory.endsWith(".sh")
    ) {
...
function prepareFixture(layout, realHome) {
  ...
  if (
    git(["status", "--porcelain=v1", "--untracked-files=all"]).toString().trim()
  ) {
  ...
  const piKetch = copyPackage(piKetchSource, join(layout.packages, "pi-ketch"));
...
function missionRecord(layout) {
  const files = readdirSync(layout.missions).filter((name) =>
    name.endsWith(".json"),
  );
  if (files.length === 0) return undefined;
  if (files.length !== 1)
    throw new Error(`Expected one acceptance Mission, found ${files.length}.`);
...
function sessionEntries(layout) {
  const files = walkFiles(layout.sessions).filter((path) =>
    path.endsWith(".jsonl"),
  );
  const calls = new Map();
...
function pendingStage(item, extra = {}) {
  return {
    ...
    ...extra,
    status: "pending",
    elapsedMs: Date.now() - new Date(item.startedAt).getTime(),
  };
}
...
function isTerminalReview(state) {
  return (
    state?.planReview?.status === "approved" ||
    state?.planReview?.status === "rejected"
  );
}
...
function findReviewUrl(logPath) {
  ...
  const matches = text.match(
    /https?:\/\/(?:127\.0\.0\.1|0\.0\.0\.0|localhost):\d+[^\s]*/gu,
  );
...
function noLargeBody(value) {
  ...
  return !/(?:planContent|planBody|planMarkdown|feedbackBody|feedbackText|browserTranscript|eventPayload)/u.test(
    serialized,
  );
}
...
function buildEvidence(layout, baseline, stages, status, extra = {}) {
  ...
  const calls = session.calls.map((call) =>
    compactCall(call, session.results.get(call.id)),
  );
  const resourceCalls = calls.filter((call) =>
    RESOURCE_NAMES.includes(call.args?.workflow),
  );
  const reviewCalls = calls.filter(
    (call) => call.name === "pi_workflow_plan_review",
  );
  ...
  const stateBytes =
    record?.statePath && existsSync(record.statePath)
      ? lstatSync(record.statePath).size
      : undefined;
  const refBytes = (value) =>
    typeof value === "string" ? Buffer.byteLength(value, "utf8") : undefined;
  const stateBoundary = state
    ? {
        discoveryBodyThroughMain:
          noLargeBody(state.discoveryMeta) &&
          !Object.hasOwn(state, "discovery"),
        researchBodyThroughMain:
          !Object.hasOwn(state, "research") &&
          !Object.hasOwn(state, "researchReport"),
        planBodyThroughMain:
          !Object.hasOwn(state, "plan") &&
          !Object.hasOwn(state, "planBody") &&
          !Object.hasOwn(state, "planContent") &&
          !Object.hasOwn(state, "planMarkdown"),
        humanFeedbackBodyThroughMain:
          !Object.hasOwn(state, "feedback") &&
          !Object.hasOwn(state, "feedbackBody") &&
          !Object.hasOwn(state, "feedbackText"),
        largeProseInMissionState:
          stateBytes === undefined ? undefined : stateBytes <= 262144,
      }
    : undefined;
  return {
    status,
    ...
    research: {
      ...
      agent:
        childSummary(mission, "research")?.agent ?? "pi-workflow.researcher",
      context: "fresh",
      async: false,
    ...
    planning: {
      decision: state?.planningDecision
        ? {
            bounded: JSON.stringify(state.planningDecision).length <= 32768,
            present: true,
          }
        : undefined,
    ...
    planReviewBinding: state?.planReview
      ? {
          ...
          feedbackRef: state.planReview.feedbackRef,
          bindingBytes: Buffer.byteLength(
            JSON.stringify(state.planReview),
            "utf8",
          ),
        }
      : undefined,
    correlation: {
      ...
      planRef: state?.planReview?.planRef ?? state?.planRef,
      reviewId: state?.planReview?.reviewId ?? planReviewId,
      prepareReview: calls.some(
        (call) =>
          call.args?.workflow === "pi-workflow.planning" &&
          call.args?.operation === "prepare-review" &&
          call.ok,
      ),
      recordReview: calls.some(
        (call) =>
          call.args?.workflow === "pi-workflow.planning" &&
          call.args?.operation === "record-review" &&
          call.ok,
      ),
      reviewStatus: calls.some(
        (call) =>
          call.args?.workflow === "pi-workflow.planning" &&
          call.args?.operation === "review-status" &&
          call.ok,
      ),
    ...
    largeDataBoundary: {
      ...
      forbiddenMainTransportFields: calls.some((call) =>
        [
          ...
          "outputSchema",
          "outputPath",
        ].some((key) => Object.hasOwn(call.args ?? {}, key)),
      )
        ? "FOUND"
        : "NONE",
    },
    recovery: {
      duplicateReviewObserved:
        (reviewCalls.length > 1 && terminalReview) || false,
      ...
      plannotatorStatuses: Object.fromEntries(
        Object.entries(statuses).map(([id, value]) => [
          id,
          { status: value?.status, approved: value?.approved },
        ]),
      ),
    },
    resourceCalls,
...
function spawnInteractive(layout, environment, piInvocation) {
  ...
  const errorLog = createWriteStream(layout.errorLog, {
    encoding: "utf8",
    mode: 0o600,
  });
...
function realPiHomeDiff(before, after) {
  ...
  return snapshotDiff(before, after).filter(
    ({ path }) => path !== currentSession,
  );
}
...
function controlValidation() {
  return new Promise((resolvePromise) => {
    execFile(
      process.execPath,
      [
        join(REPO_ROOT, "scripts", "validate-unit5-isolated.mjs"),
        "--research=skip",
        "--timeout-ms=900000",
      ],
...
export function stageState(layout, stages) {
  ...
  const discoveryChildren = [
    childSummary(mission, "discovery-artifact"),
    childSummary(mission, "discovery-metadata"),
  ].filter(Boolean);
  if (
    state?.discoveryRef &&
    state.discoveryMeta &&
    discoveryChildren.every((child) => child.status === "completed")
  ) {
    stages[1] = completeStage(stages[1], {
      ...
      metadata: {
        ...
        humanClarificationRequired:
          state.discoveryMeta.humanClarificationRequired,
      },
    });
  }
  const researchChild = childSummary(mission, "research");
  if (
    state?.researchMeta?.status === "completed" &&
    state.researchRef &&
    researchChild?.status === "completed"
  ) {
  ...
  if (
    state?.planRef &&
    state.planningDecision &&
    childSummary(mission, "planning")?.status === "completed"
  ) {
  ...
  const pendingStatus = Object.values(statuses).find(
  ...
  if (
    ...
    stages[5] = pendingStage(stages[5], {
      agent: "Main-only Plan Review bridge + Plannotator",
      runKey: "pi_workflow_plan_review",
      reviewId: Object.entries(statuses).find(
        ([, value]) => value?.status === "pending",
      )?.[0],
      planRef: state?.planRef,
  ...
  if (
    recordCall &&
    callResult(recordCall)?.isError !== true &&
    state?.planReview?.status !== "pending"
  ) {
    stages[6] = completeStage(stages[6], {
      agent: "Planning Resource",
      runKey: "record-review",
      refs: [state.planReview.planRef, state.planReview.reviewId].filter(
        Boolean,
      ),
      result: {
        status: state.planReview.status,
  ...
  if (
    statusCall &&
    callResult(statusCall)?.isError !== true &&
    state?.planReview?.status !== "pending"
  ) {
    stages[7] = completeStage(stages[7], {
      ...
      result: {
        status: state.planReview.status,
        correlation: reviewId === state.planReview.reviewId,
      },
    });
  }
...
async function runNative(options) {
  ...
  if (existsSync(accidentalExtension))
    fail(
      `BLOCKED: refusing to touch existing extension path ${accidentalExtension}`,
    );
  ...
  try {
    ...
    for (;;) {
      ...
      const pending = Object.values(statuses).some(
        (value) => value?.status === "pending",
      );
      const terminal =
        state?.planReview?.status === "approved" ||
      ...
      const complete =
        state?.planReview?.status === "approved" &&
        stages.every((item) => item.status === "completed");
      const status = complete
        ? "PASS"
        : pending
          ? "WAITING_FOR_HUMAN"
          : "RUNNING";
      const evidence = buildEvidence(layout, baseline, stages, status, {
        startedAt,
        native: {
          ...
          childRuns: [
            ...
            .map((key) => childSummary(record?.mission, key))
            .filter(Boolean),
          ...
          JSON.stringify(
            {
              ...
              reviewId,
              reviewUrl: findReviewUrl(layout.log),
              stages: stages.map(({ status: stageStatus, runKey, agent }) => ({
                status: stageStatus,
                runKey,
                agent,
              })),
      ...
      if (complete && !printedPass) {
        ...
        if (diff.length > 0)
          throw new Error(
            `Real Pi home changed during native acceptance (${diff.length} differences).`,
          );
        if (record?.mission?.status !== "active")
          throw new Error(
            `Mission was not left active: ${record?.mission?.status}`,
          );
        if (!state.researchRef || state.researchMeta?.status !== "completed")
          throw new Error("Required Research handoff is incomplete.");
        if (
          !state.discoveryRef ||
          !state.discoveryMeta ||
          state.discoveryMeta.externalResearchRequired !== true
        )
          throw new Error("Required Discovery handoff is incomplete.");
        if (!state.planRef || !existsSync(state.planRef))
          throw new Error("Canonical Plan Artifact is missing.");
        if (
          state.planReview.reviewId !== reviewId ||
          state.planReview.planRef !== state.planRef
        )
          throw new Error("Final Plan Review correlation is invalid.");
        if (evidence.largeDataBoundary.forbiddenMainTransportFields !== "NONE")
          throw new Error(
            "Large Plan/feedback transport field appeared in Main args.",
          );
        const control = await controlValidation();
        evidence.controlPath = control;
        evidence.validation = {
          realPiHomeMutation: "NONE",
      ...
      if (
        (child.exitCode !== null || !interactiveProcessActive(layout)) &&
        !pending &&
        !complete
      ) {
        throw new Error(
          `Interactive Pi exited before Unit 6 completion: code=${String(child.exitCode)}.`,
        );
      }
    }
  } catch (error) {
    stopProcess(child);
    const diff = realPiHomeDiff(before, protectedPiSnapshot(realHome));
    const failure = buildEvidence(
      layout,
      baseline,
      stages,
      diff.length > 0 ? "BLOCKED" : "BLOCKED",
      {
