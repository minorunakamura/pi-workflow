...
Keep raw child output in managed artifacts and pass references, not report bodies. Create `implementation-plan.md` and `planning-handoff.json` as Planning-owned artifacts. After the managed Plan Artifact exists, call `pi_workflow_write_handoff` exactly once with the same-directory artifact references and bounded TDD/test/constraint/non-goal metadata. The tool computes the canonical hash, validates the Handoff, and exclusively creates `planning-handoff.json`; do not write either artifact through source-edit tools. Do not write `approval`, `reviewId`, `approvalFeedback`, or `approvedPlanHash` into `planning-handoff.json`. If Handoff creation or self-validation fails, return `FAILED` with a blocker instead of returning `COMPLETED`.
