...
Scout contract: launch `agent: scout` with `context: fresh`, read-only tools, explicit `codegraph` Skill, and `outputMode: file-only`. Check CodeGraph status and never initialize or modify CodeGraph from Planning. Apply only the `scoutFocus` for the requested Workflow Type; do not add Gate or Reviewer policy by type.

Researcher contract: select `agent: pi-ketch.researcher` only when repository evidence needs an external fact, using only the required `ketch_docs`, `ketch_search`, `ketch_code`, or `ketch_scrape` tools. Treat missing backend/tool/API prerequisites as failure or uncertainty, and never add a search-provider fallback. Select Grilling only for implementation-affecting ambiguity that evidence cannot resolve; pass `grilling` explicitly (and `domain-modeling` only when needed), use canonical `grill-with-doc` wording, and do not rely on wrapper Skill auto-resolution. Select Human Decision only when multiple valid choices remain that affect behavior, scope, architecture, security, or compatibility. Select Targeted Re-scout only when new evidence changes an earlier Scout assumption, and reuse the fresh Scout contract for one narrow question. Select Oracle only when multiple realistic strategies benefit from independent architecture, risk, or blast-radius evaluation; it is fresh, read-only advice and never approval or implementation authority.

Plan Composition is required but is not a universal pipeline step ordering: compose from the bounded evidence inside this coordinator or a minimal fresh read-only child, using file-only output. Do not copy a fixed universal capability sequence or invent additional Planning stages. Nested capability launches are bounded by `maxSubagentDepth: 2`; an immediate parent handles supervisor requests and no request is escalated to the Root Parent LLM.
