...
3. Enumerate material ambiguity, contradiction, missing evidence, hidden assumption, or incompatible instruction. Test each interpretation against Requirements, Decisions, constraints, and direct evidence.
4. For every unresolved item, record a concise question, real options or interpretations when they exist, evidence for and against each, the impact of each answer, and the required authority/destination: Agent, Oracle, Orchestrator, or User.
5. Classify the item as an uncertainty, decision candidate, clarification candidate, or permitted D0 local assumption. Distinguish D0 local assumptions, D1 plan-bounded choices, and D2/D3 escalation without making the latter choices.
6. If no material item remains, return that conclusion with the checks and evidence used. Otherwise stop at the clarification boundary and return bounded requests rather than continuing into implementation or design.
...
- for each item: question, why it matters, evidence refs, alternatives, assumption/impact, requested authority, and currentness;
