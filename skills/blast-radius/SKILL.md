...
4. Classify each impact as `direct`, `indirect`, or `unknown`, and record the affected contract or behavior, confidence, and evidence. Check relevant correctness, security, compatibility, operational, and verification/review consequences without inventing risks.
5. Map material impacts to Acceptance Criteria and constraints. List the smallest checks or additional evidence needed to confirm them.
6. Return the bounded impact handoff with scope limits and residual impact uncertainty. Stop when further transitive expansion is speculative.
...
- `observations` for established impacts and `uncertainty_candidates` for unknown reach or incomplete dependency evidence.
...
Remain read-only and independent of implementation and design authority. Do not treat a text-reference count as semantic impact, silently broaden scope, mutate source, State, graph, or Artifacts, create recovery Steps, resolve uncertainty with `figure-it-out`, contact the User, or widen authority, permissions, Write Scope, or Tools. Stop and escalate when the subject or baseline is ambiguous, a critical boundary cannot be inspected, or impact remains materially unknown. `figure-it-out` is unavailable to Reviewer in Phase 1; surface the uncertainty instead.
